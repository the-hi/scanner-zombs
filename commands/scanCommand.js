...
const scanCommand = async (interaction, options) => {
    ...
    if (LeaderBoard.has(options)) {
        const embedContent = [];
        const embed = buildEmbed(`${servers[options].name} (${options}), Population - ${LeaderBoard.get(options).pop}${LeaderBoard.get(options).isFull ? "[FULL]" : ""}, ServerUptime: ${LeaderBoard.get(options).serverAge}days`, interaction);
