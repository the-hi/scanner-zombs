    ...
    onEnterWorld(data) {
        ...
        for (const users in config.userAlters) {
            ...
            if (user[this.server] && server.pop >= parseInt(user[this.server].threshold)) {
                ...
                discord.send(`${this.server} has surpassed the threshold you've set. Current population: ${server.pop}`)
                discord.send(`The threshold you've set has been deleted. alert [serverId] [threshold] to make another alert.`)
