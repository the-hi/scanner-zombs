...
class Scanner {
    ...
    onEnterWorld(data) {
        ...
        server.pop = data.allowed ? (data.players - 1) : data.players;
        ...
        for (const users in config.userAlters) {
            ...
            if (user[this.server] && server.pop >= parseInt(user[this.server].threshold)) {
                ...
                const embed1 = buildEmbed(`${this.server} has surpassed the threshold you've set. Current population: ${server.pop}`, user[this.server].interaction)
                const embed2 = buildEmbed(`The alert you've set has been deleted. alert [serverId] [threshold] to set another alert.`, user[this.server].interaction)

                discord.send({ embeds: [embed1] })
                discord.send({ embeds: [embed2] })
